...
Protocol Version: The Virtual Disk Service Remote Protocol comprises a set of [**DCOM**](#gt_distributed-component-object-model-dcom) interfaces, which are all version 0.0. The following interfaces are common to all storage management: [IEnumVdsObject](#Section_3.1.1.1), [IVdsAdviseSink](#Section_3.3.4.3), [IVdsAsync](#Section_3.1.3.1), [IVdsService](#Section_3.1.5.1), and [IVdsServiceInitialization](#Section_3.1.5.2).<3>
...
Unless otherwise specified, all methods MUST return zero or a non-error [**HRESULT**](#gt_hresult) ([MS-ERREF](../MS-ERREF/MS-ERREF.md)) to indicate success or return an implementation-specific nonzero error code to indicate failure. For the HRESULT values predefined by the Virtual Disk Service Remote Protocol, see section [2.2.3](#Section_2.2.3).
...
The ULONGLONG data type is defined in [MS-DTYP](../MS-DTYP/MS-DTYP.md).

<a id="Section_2.2.1.1.2"></a>
##### 2.2.1.1.2 DWORD

The DWORD data type is defined in [MS-DTYP](../MS-DTYP/MS-DTYP.md).
...
The ACCESS_MASK data type is defined in [MS-DTYP](../MS-DTYP/MS-DTYP.md) section 2.4.3.
...
- **MBR basic disk volumes** - The flag is applied to all volumes created on the disk after the flag is set. It is set per disk for basic [**MBR**](#gt_master-boot-record-mbr) disks, not per volume. The flag is only set or cleared if an explicit call is made to the [IVdsVolume::SetFlags (section 3.4.5.2.32.10)](#Section_3.4.5.2.32.10) and [IVdsVolume::ClearFlag (section 3.4.5.2.32.11)](#Section_3.4.5.2.32.11) methods, respectively. For example, the **VDS_VF_NO_DEFAULT_DRIVE_LETTER** flag is not toggled as drive letters are assigned to or removed from specific volumes.
...
This section lists data types that are used exclusively by methods in the [IVdsService](#Section_3.1.5.1) interface.
...
This section lists data types that the [IVdsHbaPort](#Section_3.1.6.1) methods of the [IVdsHbaPort](#Section_3.1.6.1) interface use exclusively.
...
This section lists data types that are used exclusively by the [IVdsIscsiInitiatorAdapter](#Section_3.1.7.1) methods of the [IVdsIscsiInitiatorAdapter](#Section_3.1.7.1) interface.
...
This section lists data types that are used exclusively by the [IVdsDisk](#Section_2.2.2.9) methods.
...
This section lists data types that are used exclusively by the [IVdsAdvancedDisk](#Section_3.1.12.4) methods.
...
This section lists data types that are used exclusively by the [IVdsAdvancedDisk2](#Section_3.1.12.5) methods.
...
This section lists data types that are used exclusively by the [IVdsVolume](#Section_3.1.13.1) methods.
...
This section lists data types that are used exclusively by the [IVdsVolumeMF](#Section_3.1.13.3) methods.
...
This section lists data types that are used exclusively by the [IVdsVolumePlex](#Section_3.1.14.1) methods.
...
This section lists data types that are used exclusively by methods in the [IVdsVdProvider (section 3.1.9.4)](#Section_3.1.9.4) interface.
...
This section lists data types that are used exclusively by methods in the [IVdsVDisk](#Section_3.1.15.1) interface.
...
This section lists data types that are used exclusively by methods in the [IVdsOpenVDisk](#Section_3.1.15.2) interface.
...
The following [**HRESULT**](#gt_hresult) return codes are defined by the Virtual Disk Service Remote Protocol and, together with the HRESULTs specified in [MS-ERREF](../MS-ERREF/MS-ERREF.md), SHOULD be returned by the server to indicate additional information about the result of a method call or about the reason a call failed. If the result is an error rather than simple status information, the most significant bit of the HRESULT is set.
...
- Service object interfaces: [Service Object Interfaces (section 3.1.5)](#Section_3.1.5) through [HBA Port Object Interfaces (section 3.1.6)](#Section_3.1.6).
...
Unless otherwise specified, all methods MUST return zero or a nonerror HRESULT ([MS-ERREF](../MS-ERREF/MS-ERREF.md)) on success, or an implementation-specific nonzero error code on failure. For more details on the HRESULT values predefined by the Virtual Disk Service Remote Protocol see section [2.2.3](#Section_2.2.3).
...
The IEnumVdsObject methods are specified in section [3.4.5.2.1](#Section_3.4.5.2.1).
...
[IVdsAdviseSink](#Section_3.3.4.3) methods are specified in section 3.3.4.3.
...
The IVdsAsync methods are specified in section [3.4.5.2.2](#Section_3.4.5.2.2).
...
The IVdsServiceLoader methods are specified in section [3.4.5.2.3](#Section_3.4.5.2.3).
...
| Method | Description |
| --- | --- |
| [LoadService](#Section_3.4.5.2.3.1) | Loads the VDS service on the machine that is specified by an input parameter and returns a pointer to the [IVdsService interface](#Section_3.1.5.1). Opnum: 3 |
...
The IVdsService methods are specified in section [3.4.5.2.4](#Section_3.4.5.2.4). A method is not listed for [**opnum**](#gt_opnum) 7 because the Virtual Disk Service Remote Protocol does not use it. Attempting to call a method with opnum 7 can result in NDR raising a RPC_X_BAD_STUB_DATA exception. For more details, see [MS-DCOM](../MS-DCOM/MS-DCOM.md).
...
| [SetFlags](#Section_3.4.5.2.4.15) | Assigns property flags to the server. Opnum: 18 |
| [ClearFlags](#Section_3.4.5.2.4.16) | Clears property flags from the service. Opnum: 19 |
...
| Method | Description |
| --- | --- |
| [Initialize](#Section_3.4.5.2.5.1) | Starts the initialization of the server. Opnum: 3 |
...
| Method | Description |
| --- | --- |
| [GetProperties](#Section_3.4.5.2.11.1) | Retrieves the properties of the HBA port that is represented by the object exposing this interface and method. Opnum: 3 |
...
| Method | Description |
| --- | --- |
| [GetProperties](#Section_3.4.5.2.12.1) | Retrieves the properties of the initiator adapter that is represented by the object exposing this interface and method. Opnum: 3 |
...
| Method | Description |
| --- | --- |
| [GetProperties](#Section_3.4.5.2.13.1) | Retrieves the properties of the initiator portal that is represented by the object that exposes this interface and method. Opnum: 3 |
...
The IVdsProvider methods are specified in section [3.4.5.2.14](#Section_3.4.5.2.14).

Methods in RPC Opnum Order

| Method | Description |
| --- | --- |
| [GetProperties](#Section_3.4.5.2.14.1) | Retrieves the properties of the provider that is represented by the object exposing this interface and method. Opnum: 3 |
...
The [IVdsVdProvider](#Section_3.1.9.4) methods are specified in section 3.4.5.2.17.
...
| [AddVDisk](#Section_3.4.5.2.17.3) | Adds a virtual disk object for the specified virtual disk file to the provider and returns an [IVdsVDisk](#Section_3.1.15.1) interface pointer to it. Opnum: 5 |
| [GetDiskFromVDisk](#Section_dd2e22ab4631481fa3aa07f4b30a4d81) | Returns an [IVdsDisk](#Section_3.1.15.1) interface pointer for a virtual disk object given an IVdsVDisk interface pointer. The virtual disk MUST be [**attached**](#gt_attach). Opnum: 6 |
...
The IVdsSubSystemImportTarget methods are specified in section [3.4.5.2.18](#Section_3.4.5.2.18).
...
The IVdsPack methods are specified in section [3.4.5.2.19](#Section_3.4.5.2.19). No method with opnum 10 is listed because it is not used by this protocol.

Methods in RPC Opnum Order

| Method | Description |
| --- | --- |
| [GetProperties](#Section_3.4.5.2.19.1) | Retrieves the properties of the disk pack that is represented by the object exposing this interface and method. Opnum: 3 |
...
The IVdsDisk methods are specified in section [3.4.5.2.21](#Section_3.4.5.2.21).
...
| [GetPack](#Section_3.4.5.2.21.2) | Retrieves the [**disk pack**](#gt_disk-pack) to which the disk belongs. Opnum: 4 |
...
The IVdsDisk2 methods are specified in section [3.4.5.2.22](#Section_3.4.5.2.22).
...
The IVdsAdvancedDisk methods are specified in section [3.4.5.2.24](#Section_3.4.5.2.24).
...
The IVdsAdvancedDisk2 methods are specified in section [3.4.5.2.25](#Section_3.4.5.2.25).
...
| Method | Description |
| --- | --- |
| [GetProperties](#Section_3.4.5.2.26.1) | Retrieves the properties of the disk that is represented by the object exposing this interface and method. Opnum: 3 |
...
| Method | Description |
| --- | --- |
| [Online](#Section_3.4.5.2.28.1) | Brings the disk to the online state. An online disk exposes the [**volumes**](#gt_volume) on that disk. Opnum: 3 |
...
The IVdsVolume methods are specified in section [3.4.5.2.32](#Section_3.4.5.2.32).
...
| [GetPack](#Section_3.4.5.2.32.2) | Retrieves the [**disk pack**](#gt_disk-pack) to which the volume belongs. Opnum: 4 |
...
| [Shrink](#Section_3.4.5.2.32.5) | Reduces the size of the volume and all plexes and returns the released extents to [**free space**](#gt_free-space). Opnum: 7 |
...
| [ClearFlags](#Section_3.4.5.2.32.11) | Clears flags from a volume. This method is not implemented for removable disks. Opnum: 13 |
...
The IVdsVolume2 methods are specified in section [3.4.5.2.33](#Section_3.4.5.2.33).
...
The IVdsVolumeMF methods are specified in section [3.4.5.2.34](#Section_3.4.5.2.34).
...
The [IVdsVolumeMF2](#Section_3.1.13.4) methods are specified in section 3.4.5.2.35.
...
The IVdsVolumeMF3 methods are specified in section [3.4.5.2.36](#Section_3.4.5.2.36).
...
The IVdsVolumeShrink methods are specified in section [3.4.5.2.37](#Section_3.4.5.2.37).
...
| [Shrink](#Section_3.4.5.2.37.2) | Shrinks the volume and all [**plexes**](#gt_plex) and returns the released extents. Opnum: 4 |
...
| Method | Description |
| --- | --- |
| [Online](#Section_3.4.5.2.38.1) | Brings the volume online. Opnum: 3 |
...
The IVdsVolumePlex methods are specified in section [3.4.5.2.39](#Section_3.4.5.2.39).
...
The IVdsVDisk methods are specified in section [3.4.5.2.40](#Section_3.4.5.2.40).

Methods in RPC Opnum Order

| Method | Description |
| --- | --- |
| [Open](#Section_3.4.5.2.40.1) | Opens a handle to the specified virtual disk file and returns an [IVdsOpenVDisk](#Section_3.1.15.2) interface pointer to the object that represents the opened handle. Opnum: 3 |
...
The IVdsOpenVDisk methods are specified in section [3.4.5.2.41](#Section_3.4.5.2.41).
...
Service Loader and Service: The first interface the client obtains is the [IVdsServiceLoader](#Section_3.1.4.1) interface. The client invokes IVdsServiceLoader::LoadService to load the [**VDS**](#gt_virtual-disk-service-vds) service on the specified machine. The server responds with an [IVdsService](#Section_3.1.5.1) interface for the VDS service that is loaded.
...
Virtual Disk Providers and Virtual Disks: Similar to hardware-providers, the client invokes IVdsService::QueryProviders with the flag VDS_QUERY_VIRTUALDISK_PROVIDERS to obtain a list of VDS [**virtual disk**](#gt_virtual-disk) providers, each of which implement an [IVdsVdProvider](#Section_3.1.9.4) interface. The client invokes the IUnknown QueryInterface method to retrieve an IVdsVdProvider interface on the virtual disk provider object. The client then invokes [IVdsVdProvider::QueryVDisks](#Section_3.4.5.2.17.1) to obtain a list of virtual disks that the provider maintains. The server responds with a IEnumVdsObject interface, which enumerates a list of IUnknown interfaces, one for each virtual disk. The client invokes IUnknown::QueryInterface to retrieve an [IVdsVDisk](#Section_3.1.15.1) interface on the virtual disk object.

Service and [**Unallocated Disks**](#gt_unallocated-disk): The client invokes IVdsService::QueryUnallocatedDisks to obtain a list of disks that do not have a recognized disk partitioning [**format**](#gt_format). The server responds with an IEnumVdsObject interface, which enumerates a list of IUnknown interfaces, one for each unallocated disk that is available on the server. The client invokes IUnknown::QueryInterface to retrieve an [IVdsDisk](#Section_3.1.15.1), [IVdsDisk2](#Section_3.1.12.2), [IVdsDisk3](#Section_3.1.12.3), [IVdsAdvancedDisk](#Section_3.1.12.4), [IVdsAdvancedDisk2](#Section_3.1.12.5), [IVdsAdvancedDisk3](#Section_3.1.12.6), [IVdsDiskPartitionMF](#Section_3.1.12.9), [IVdsDiskPartitionMF2](#Section_3.4.5.2.30), [IVdsCreatePartitionEx](#Section_3.1.12.7), [IvdsDiskOnline,](#Section_3.1.12.8) or [IVdsRemovable](#Section_3.4.5.2.31) interface on the disk object.

Service and Disks: The client invokes IUnknown::QueryInterface on the IVdsService interface to retrieve the [IVdsServiceUninstallDisk](#Section_3.1.5.3) interface. The client invokes [IVdsServiceUninstallDisk::UninstallDisks](#Section_3.4.5.2.6.2) to uninstall one or more disks that are installed on the server. See section [3.2.1.4](#Section_3.2.1.4). The client invokes IUnknown::QueryInterface on the IVdsService interface to retrieve the [IVdsServiceSAN](#Section_3.1.5.6) interface. The client invokes [IVdsServiceSAN::GetSANPolicy](#Section_3.4.5.2.9.1) to query the current SAN policy setting, and [IVdsServiceSAN::SetSANPolicy](#Section_3.4.5.2.9.2) to set the SAN policy.
...
Volumes and Supported File System Formats: The client invokes IUnknown::QueryInterface to retrieve an [IVdsVolumeMF2](#Section_3.1.13.4) interface. The client invokes [IVdsVolumeMF2::QueryFileSystemFormatSupport](#Section_3.4.5.2.35.2) to obtain a list of file systems that are supported for the volume. The server returns a list of [VDS_FILE_SYSTEM_FORMAT_SUPPORT_PROP](#Section_2.2.1.3.18) structures, one for each file system that is supported on the volume.
...
Prior to invoking CreatePartition, the client invokes IVdsAdvancedDisk::GetPartitionProperties to retrieve the [**partition type**](#gt_partition-type) (partition style) for the [**disk**](#gt_disk). The client passes this value as the [CREATE_PARTITION_PARAMETERS](#Section_2.2.1.3.22)::**style** input parameter to the CreatePartition method. [IVdsAdvancedDisk](#Section_3.1.12.4)::GetPartitionProperties returns this value as the [VDS_PARTITION_PROP](#Section_2.2.1.3.22)::**PartitionStyle** structure member.
...
[IVdsAdvancedDisk::FormatPartition](#Section_3.4.5.2.24.9): For call sequencing related to the *ullOffset* input parameter, see the description for IVdsAdvancedDisk::DeletePartition. For call sequencing, see sections [3.2.1.1.2](#Section_3.2.1.1.2), [3.4.1.4](#Section_3.4.1.4), [3.4.1.5](#Section_3.4.1.5), and [4.5](#Section_4.5).
...
After the client obtains the IVdsServiceLoader interface, the client MUST invoke [IVdsServiceLoader::LoadService](#Section_3.4.5.2.3.1) on the interface to retrieve the [IVdsService](#Section_3.1.5.1) interface.
...
- After the client obtains the IVdsServiceInitialization interface, the client MUST invoke the [IVdsServiceInitialization::Initialize](#Section_3.4.5.2.5.1) method on the interface before invoking any other method.
...
The client SHOULD choose to implement the [IVdsAdviseSink](#Section_3.3.4.3) interface in order to receive notification from the server of changes to the storage objects on the server. Notifications are sent to the client for creating, deleting, and modifying storage objects. The client can choose to take other action based on these notifications. The client can also choose to ignore notifications from the server.
...
**Return Values:** The method MUST return zero or a non-error [**HRESULT**](#gt_hresult) (as specified in [MS-ERREF](../MS-ERREF/MS-ERREF.md)) to indicate success or return an implementation-specific nonzero error code to indicate failure. For the HRESULT values predefined by the Virtual Disk Service Remote Protocol, see section [2.2.3](#Section_2.2.3).
...
Unless otherwise specified, all methods MUST return zero or a non-error [**HRESULT**](#gt_hresult) (as specified in [MS-ERREF](../MS-ERREF/MS-ERREF.md)) to indicate success, or return an implementation-specific nonzero error code to indicate failure. For the HRESULT values predefined by the Virtual Disk Service Remote Protocol, see section [2.2.3](#Section_2.2.3).
...
- Implements [IVdsService](#Section_3.1.5.1) and [IVdsServiceInitialization](#Section_3.1.5.2) interfaces.
- Implements [IVdsServiceUninstallDisk](#Section_3.1.5.3), [IVdsServiceHba](#Section_3.1.5.4), [IVdsServiceSAN](#Section_3.1.5.6), and [IVdsServiceIscsi](#Section_3.1.5.5) interfaces.<66>
...
- Each HBA port object implements the [IVdsHbaPort](#Section_3.1.6.1) interface.
- Initiator Adapter
...
- Each initiator adapter object implements the [IVdsIscsiInitiatorAdapter](#Section_3.1.7.1) interface.
- Initiator Portal
...
- Each software provider object implements the [IVdsProvider](#Section_3.1.9.1) and [IVdsSwProvider](#Section_3.1.9.2) interfaces.
- Hardware Provider
...
- Each hardware provider object implements the IVdsProvider and [IVdsVdProvider](#Section_3.1.9.4) interfaces.
- Subsystem
...
- Each pack object implements the [IVdsPack2](#Section_3.1.11.2) interface.
...
- Each disk object implements the [IVdsDisk](#Section_3.1.15.1) and [IVdsAdvancedDisk](#Section_3.1.12.4) interfaces.
...
- Each disk object implements the [IVdsDisk2](#Section_3.1.12.2), [IVdsDisk3](#Section_3.1.12.3), [IVdsAdvancedDisk2](#Section_3.1.12.5), [IVdsAdvancedDisk3](#Section_3.1.12.6), [IVdsCreatePartitionEx](#Section_3.1.12.7), [IVdsDiskPartitionMF](#Section_3.1.12.9), [IVdsDiskPartitionMF2](#Section_3.4.5.2.30), and [IvdsDiskOnline](#Section_3.1.12.8) interfaces.
...
- Each volume object implements the [IVdsVolume](#Section_3.1.13.1), [IVdsVolume2](#Section_3.1.13.2), [IVdsVolumeMF](#Section_3.1.12.9), [IVdsVolumeMF2](#Section_3.1.13.4), [IVdsVolumeMF3](#Section_3.1.13.5), [IVdsVolumeShrink](#Section_3.1.13.6), and [IVdsVolumeOnline](#Section_3.1.13.7) interfaces.
...
- Each volume plex object implements the [IVdsVolumePlex](#Section_3.1.14.1) interface.
...
- Each virtual disk object implements the [IVdsVDisk](#Section_3.1.15.1) interface.
- When a virtual disk is opened, it MUST create an object to represent the open virtual disk (an OpenVirtualDisk object), which MUST implement the [IVdsOpenVDisk](#Section_3.1.15.2) interface.
...
Callback Object Interface: A pointer to the [IVdsAdviseSink](#Section_3.3.4.3) interface that is implemented by the callback object that is used to receive notifications from the server.
...
If service initialization has not started when the client calls the IVdsServiceInitialization::Initialize (Opnum 3) (section [3.4.5.2.5.1)](#Section_3.4.5.2.5.1) method, the service MUST start initializing.
...
**Return Values:** The method MUST return zero or a non-error HRESULT (as specified in [MS-ERREF](../MS-ERREF/MS-ERREF.md)) to indicate success or return an implementation-specific nonzero error code to indicate failure. For the HRESULT values predefined by the Virtual Disk Service Remote Protocol, see section [2.2.3](#Section_2.2.3).
...
**ppService:** A pointer to the [IVdsService](#Section_3.1.5.1) interface that, if successfully completed, returns the IVdsService interface to the VDS service that runs on the machine represented by *pwszMachineName*.
...
- Call the [IVdsServiceInitialization::Initialize](#Section_3.4.5.2.5.1) method, passing the *pwszMachineName* parameter input to this method.
...
- Verify that *ppEnum* is not NULL.
The server MUST point *ppEnum* to an IEnumVdsObject interface that contains the enumeration of provider objects in the list of cached storage management objects, as specified in section [3.4.1.3](#Section_3.4.1.3), and return an HRESULT that indicates failure or success.
...
- Verify that *ppEnum* is not NULL.
The server MUST point *ppEnum* to an IEnumVdsObject interface that contains the enumeration of unallocated disk objects in the list of cached storage management objects, as specified in section [3.4.1.3](#Section_3.4.1.3), and return an HRESULT indicating failure or success.
...
**pSink:** A pointer to an [IVdsAdviseSink](#Section_3.3.4.3) interface of the callback object to register with the server for notification of object changes.
...
**dwCookie:** The cookie value generated when the [IVdsAdviseSink](#Section_3.3.4.3) interface was registered.
...
**Return Values:** The method MUST return zero or a nonerror [**HRESULT**](#gt_hresult) (as specified in [MS-ERREF](../MS-ERREF/MS-ERREF.md)) to indicate success, or return an implementation-specific nonzero error code to indicate failure. For the HRESULT values predefined by the Virtual Disk Service Remote Protocol, see section [2.2.3](#Section_2.2.3).
...
- Verify that *ppEnum* is not NULL.
The server MUST point *ppEnum* to an IEnumVdsObject interface that contains the enumeration of the HBA port objects in the list of cached storage management objects, as specified in section [3.4.1.3](#Section_3.4.1.3), and return an HRESULT indicating failure or success.
...
- Verify that *ppEnum* is not NULL.
The server MUST point *ppEnum* to an IEnumVdsObject interface that contains the enumeration of iSCSI initiator adapter objects in the list of cached storage management objects, as specified in section [3.4.1.3](#Section_3.4.1.3), and return an HRESULT indicating failure or success.
...
**Return Values:** The method MUST return zero or a non-error [**HRESULT**](#gt_hresult) (as specified in [MS-ERREF](../MS-ERREF/MS-ERREF.md)) to indicate success, or return an implementation-specific nonzero error code to indicate failure.<76> For the HRESULT values predefined by the Virtual Disk Service Remote Protocol, see section [2.2.3](#Section_2.2.3).
...
**ppInitiatorAdapter:** A pointer to an [IVdsIscsiInitiatorAdapter](#Section_3.1.7.1) interface that, if the operation is successfully completed, receives the IVdsIscsiInitiatorAdapter interface of the initiator adapter object that the initiator portal belongs to. Callers MUST release the interface when they are done with it.
...
- Verify that *ppEnum* is not NULL.
The server MUST point *ppEnum* to an IEnumVdsObject interface that contains the enumeration of pack objects in the list of cached storage management objects that have their provider pointer set to this provider object. The server MUST then return an HRESULT indicating failure or success. For details on enumeration objects, see section [3.4.1.3](#Section_3.4.1.3).
...
- Verify that *ppEnum* is not NULL.
The server MUST point *ppEnum* to an IEnumVdsObject interface that contains the enumeration of subsystem objects in the list of cached storage management objects that have their provider pointer set to this provider object. The server MUST then return an HRESULT to indicate failure or success. For details on enumeration objects, see section [3.4.1.3](#Section_3.4.1.3).
...
The AddVDisk method creates a [**virtual disk**](#gt_virtual-disk) object representing the specified virtual disk and adds it to the list of virtual disks managed by the provider. This method returns an [IVdsVDisk (section 3.1.15.1)](#Section_3.1.15.1) interface pointer to the specified virtual disk object.
...
The GetDiskFromVDisk method returns an [IVdsDisk (section 3.1.12.1)](#Section_3.1.12.1) interface pointer for a [**virtual disk**](#gt_virtual-disk) given an [IVdsVDisk (section 3.1.15.1)](#Section_3.1.15.1) interface pointer.
...
The GetVDiskFromDisk method returns an [IVdsVDisk (section 3.1.15.1)](#Section_3.1.15.1) interface pointer for the [**virtual disk**](#gt_virtual-disk) given an [IVdsDisk (section 3.1.12.1)](#Section_3.1.12.1) interface pointer.
...
- Verify that *ppEnum* is not NULL.
The server MUST point *ppEnum* to an IEnumVdsObject interface that contains the enumeration of volume objects in the list of cached storage management objects that have their pack pointer set to this pack object. The server MUST then return an HRESULT indicating failure or success. For details on enumeration objects, see section [3.4.1.3](#Section_3.4.1.3).
...
- Verify that **ppEnum** is not NULL.
The server MUST point **ppEnum** to an IEnumVdsObject interface that contains the enumeration of disk objects in the list of cached storage management objects that have their pack pointer set to this pack object. The server MUST then return an HRESULT indicating failure or success. For details on enumeration, see section [3.4.1.3](#Section_3.4.1.3).
...
- Create a new volume object that corresponds to the new volume, implement the [IVdsVolume](#Section_3.1.13.1) interface, and assign it a unique [VDS_OBJECT_ID](#Section_2.2.1.1.3).
...
**Return Values:** The method MUST return zero or a non-error [**HRESULT**](#gt_hresult) (as specified in [MS-ERREF](../MS-ERREF/MS-ERREF.md)) to indicate success, or return an implementation-specific nonzero error code to indicate failure. For the HRESULT values predefined by the Virtual Disk Service Remote Protocol, see section [2.2.3](#Section_2.2.3).<94>
...
**Return Values:** The method MUST return zero or a non-error [**HRESULT**](#gt_hresult) (as specified in [MS-ERREF](../MS-ERREF/MS-ERREF.md)) to indicate success, or return an implementation-specific nonzero error code to indicate failure. For the HRESULT values predefined by the Virtual Disk Service Remote Protocol, see section [2.2.3](#Section_2.2.3).<107>
...
- Verify that **ppEnum** is not NULL.
The server MUST point **ppEnum** to an IEnumVdsObject interface that contains the enumeration of volume plex objects in the list of cached storage management objects that have their volume pointer set to this volume object, as specified in section [3.4.1.3](#Section_3.4.1.3). The server MUST then return an HRESULT indicating failure or success.
...
- Create a new volume object that corresponds to the new volume, implements the [IVdsVolume](#Section_3.1.13.1) interface, and assigns it a unique VDS_OBJECT_ID.
...
**Return Values:** The method MUST return zero or a non-error HRESULT (as specified in [MS-ERREF](../MS-ERREF/MS-ERREF.md)) to indicate success, or return an implementation-specific nonzero error code to indicate failure. For the [**HRESULT**](#gt_hresult) values predefined by the Virtual Disk Service Remote Protocol, see section [2.2.3](#Section_2.2.3).
...
**ppVolume:** A pointer to an [IVdsVolume](#Section_3.1.13.1) interface that, if the operation is successfully completed, receives the IVdsVolume interface of the volume object that the volume plex belongs to. Callers MUST release the interface when they are done with it.
...
The Open method opens a handle to the specified [**virtual disk**](#gt_virtual-disk) file and returns an [IVdsOpenVDisk (section 3.1.15.2)](#Section_3.1.15.2) interface pointer to an object representing the open virtual disk (an OpenVirtualDisk object). Release the IVdsOpenVDisk interface to close the handle to the virtual disk.
...
**AccessMask:** A [VIRTUAL_DISK_ACCESS_MASK (section 2.2.2.19.1.4)](#Section_2.2.2.19.1.4) structure that contains the set of access rights to be applied to the opened virtual disk.
...
- Mark the state of the object that implements [IVdsVDisk](#Section_3.4.5.2.40) as "open". For details, see section [2.2.2.19.1.1](#Section_2.2.2.19.1.1).
...
**ppVolume:** Pointer to a variable that receives an [IVdsVolume (section 3.1.13.1)](#Section_3.1.13.1) interface pointer for the volume. Callers MUST release the interface pointer when it is no longer needed by calling the **IUnknown::Release** method.
...
- The server returns a reference to the [IVdsService](#Section_3.1.5.1) interface.
...
- The client calls the [IVdsServiceInitialization::Initialize](#Section_3.4.5.2.5.1) method.
...
- If the client wants to query the objects in the provider, the client invokes the interface's IUnknown::QueryInterface method to request for the object's [IVdsSwProvider](#Section_3.1.9.2) interface.
...
- The client releases the reference to the [IVdsService](#Section_3.1.5.1) interface by invoking IVdsService::Release.
...
- Assuming the server returned a zero HRESULT, the client invokes the interface's IUnknown::QueryInterface method to request for the object's [IVdsVolume](#Section_3.1.13.1) interface.
...
For ease of implementation, the full [**IDL**](#gt_interface-definition-language-idl) is provided here; "ms-dtyp.idl" is the IDL that is in [MS-DTYP](../MS-DTYP/MS-DTYP.md) section 5.
...
| [IVdsSwProvider](#Section_3.1.9.2) | X | X | X | X | X | X |
| [IVdsPack](#Section_2.2.2.8) | X | X | X | X | X | X |
| [IVdsDisk](#Section_3.1.15.1) | X | X | X | X | X | X |
| [IVdsAdvancedDisk](#Section_3.1.12.4) | X | X | X | X | X | X |
| [IVdsCreatePartitionEx](#Section_3.1.12.7) | X | X | X | X | X | X |
| [IVdsRemovable](#Section_3.4.5.2.31) | X | X | X | X | X | X |
| [IVdsVolume](#Section_3.1.13.1) | X | X | X | X | X | X |
| IVdsVolume2 | - | - | - | - | X | X |
| [IVdsVolumeMF](#Section_3.1.12.9) | X | X | X | X | X | X |
| [IVdsVolumePlex](#Section_3.1.14.1) | X | X | X | X | X | X |
| [IVdsServiceUninstallDisk](#Section_3.1.5.3) | - | X | X | X | X | X |
| [IVdsPack2](#Section_3.1.11.2) | - | - | X | X | X | X |
| [IVdsDisk2](#Section_3.1.12.2) | - | - | X | X | X | X |
| IVdsDisk3 | - | - | - | - | X | X |
| [IVdsAdvancedDisk2](#Section_3.1.12.5) | - | - | X | X | X | X |
| [IVdsAdvancedDisk3](#Section_3.1.12.6) | - | - | - | - | - | X |
| [IVdsDiskPartitionMF](#Section_3.1.12.9) | - | - | X | X | X | X |
| IVdsDiskPartitionMF2 | - | - | - | - | X | X |
| [IVdsVolumeMF2](#Section_3.1.13.4) | - | - | X | X | X | X |
| IVdsVolumeMF3 | - | - | - | - | X | X |
| [IVdsVolumeShrink](#Section_3.1.13.6) | - | - | X | X | X | X |
| [IVdsVolumeOnline](#Section_3.1.13.7) | - | - | - | - | X | X |
| [IVdsHwProvider](#Section_3.1.9.3) | X | X | X | X | X | X |
| [IVdsServiceLoader](#Section_3.1.4.1) | X | X | X | X | X | X |
| [IVdsVdProvider](#Section_3.1.9.4) | - | - | - | - | X | X |
| [IVdsVDisk](#Section_3.1.15.1) | - | - | - | - | X | X |
| [IVdsOpenVDisk](#Section_3.1.15.2) | - | - | - | - | X | X |
...
[IVdsVolumeMF3](#Section_3.1.13.5) extends IVdsVolumeMF with functionality that is related to file systems.
...
The following interfaces, used when querying HBA and iSCSI initiator information, are not supported in Windows Server 2003: [IVdsServiceHba](#Section_3.1.5.4), [IVdsServiceIscsi](#Section_3.1.5.5), [IVdsHbaPort](#Section_3.1.6.1), [IVdsIscsiInitiatorAdapter](#Section_3.1.7.1), and [IVdsIscsiInitiatorPortal](#Section_3.1.8.1).
...
<14> Section 2.2.1.2.23: For Windows Server 2003 and Windows Server 2003 R2, **VDS_VF_NO_DEFAULT_DRIVE_LETTER** cannot be set or cleared on [**dynamic disk**](#gt_dynamic-disk) volumes, and is always enabled. On [**basic disks**](#gt_basic-disk), it is disabled by default, and the flags can only be changed using the [IVdsVolume::SetFlags (section 3.4.5.2.32.10)](#Section_3.4.5.2.32.10) and [IvdsVolume::ClearFlag (section 3.4.5.2.32.11)](#Section_3.4.5.2.32.11) methods. Assigning or removing a [**drive letter**](#gt_drive-letter) does not toggle the flag for basic GPT disks.
...
<137> Section 3.4.5.2.37.2: The [Shrink](#Section_3.4.5.2.37.2) method works only on volumes that have NTFS or RAW file systems; otherwise, Shrink returns VDS_E_CANNOT_SHRINK (HRESULT of 0x8004251E).
