...
| HTTP method | Description |
| --- | --- |
| [GET](#Section_3.1.5.2.1) | For a description, see section [3.2.5.2](#Section_3.1.5). |
...
After receiving the request, the server verifies the signature of the request and also verifies that the **request_nonce** is a nonce value previously issued by the server as defined in section [3.2.5.1.1](#Section_3.2.5.1.1.1). The server SHOULD also verify that the nonce was issued recently.<5> If the signature or nonce are invalid, the server returns the error "invalid_grant" using the format described in [[RFC6749]](https://go.microsoft.com/fwlink/?LinkId=301486) section 5.2.
...
The [**AD FS server**](#gt_ad-fs-server) processes the [x-ms-RefreshTokenCredential HTTP header](#Section_2.2.1.1) as follows.
...
- The server verifies the signature of the header and also verifies that the **request_nonce** is a nonce value previously issued by the server as defined in section [3.2.5.1.1](#Section_3.2.5.1.1.1). The server SHOULD<12> also verify that the nonce was issued recently. If the signature or **request_nonce** are invalid, the server ignores the x-ms-RefreshTokenCredential HTTP header; if the [x-ms-DeviceCredential HTTP header](#Section_2.2.1.2) is present, the client processes it as follows, otherwise it continues processing the request as in [MS-OAPX] section 3.2.5.1.1.3.
