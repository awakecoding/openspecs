...
The RegisterResponse message is sent by the [**resolver service**](#gt_resolver-service) in response to a [Register](#Section_3.1.4.1) or [Update](#Section_2.2.3.4) message. It is only sent to the originator of the Register or Update message, and it indicates the result of an attempt to either create a new [**registration record**](#gt_registration-record) or to update an existing one.
...
The ResolveResponse message is sent by the [**resolver service**](#gt_resolver-service) in response to a [Resolve](#Section_2.2.3.2) message. It contains [**endpoint information**](#gt_endpoint-information) that corresponds to the details of the Resolve request.
...
- **RegistrationId**: A [**GUID**](#gt_globally-unique-identifier-guid) ([MS-DTYP](../MS-DTYP/MS-DTYP.md) section 2.3.4) that identifies the specific [**registration record**](#gt_registration-record) that is stored at the resolver service and is used to refer to that record for the [Update](#Section_2.2.3.4), [Unregister](#Section_3.1.4.4), and [Refresh](#Section_2.2.3.3) operations. The **RegistrationId** element is contained in the [RegisterResponse](#Section_2.2.4.1) message returned by the resolver service as a result of a successful **Register** operation and is generated by the resolver service.
...
This timer is used to determine when to call [Refresh](#Section_2.2.3.3). The client MUST ensure that the **Refresh** operation is called before the time indicated by the **RegistrationLifetime** element has elapsed. This value is received as part of the [RegisterResponse](#Section_2.2.4.1) message in response to the initial [Register](#Section_3.1.4.1) request or any subsequent [Update](#Section_2.2.3.4) request.
...
The Register message stores [**endpoint information**](#gt_endpoint-information) at the [**resolver service**](#gt_resolver-service). The [**resolver client**](#gt_resolver-client) MUST follow the procedure outlined in section [3.1.4](#Section_3.1.4) and MUST send a [Register](#Section_3.1.4.1) message to the resolver service.

<a id="Section_3.1.4.2"></a>
#### 3.1.4.2 Update

The Update message modifies an existing [**registration record**](#gt_registration-record) stored at the [**resolver service**](#gt_resolver-service). The [**resolver client**](#gt_resolver-client) MUST follow the procedure outlined in section [3.1.4](#Section_3.1.4) and MUST send an [Update](#Section_2.2.3.4) message to the resolver service.

<a id="Section_3.1.4.3"></a>
#### 3.1.4.3 Resolve

The Resolve operation retrieves registration information corresponding to a specific [**mesh name**](#gt_mesh-name). The [**resolver client**](#gt_resolver-client) MUST follow the procedure outlined in section [3.1.4](#Section_3.1.4) and MUST send a [Resolve](#Section_2.2.3.2) message to the [**resolver service**](#gt_resolver-service).

<a id="Section_3.1.4.4"></a>
#### 3.1.4.4 Unregister

The [Unregister](#Section_3.1.4.4) message removes a specific [**registration record**](#gt_registration-record) from the [**resolver service**](#gt_resolver-service). The [**resolver client**](#gt_resolver-client) MUST follow the procedure outlined in section [3.1.4](#Section_3.1.4), with the exception that it MUST NOT start a message response timer. It MUST send an Unregister message to the resolver service and SHOULD close the connection to the resolver service after successfully sending the Unregister message.

<a id="Section_3.1.4.5"></a>
#### 3.1.4.5 Refresh

The Refresh message extends the [**registration lifetime**](#gt_registration-lifetime) of a specific registration. The [**resolver client**](#gt_resolver-client) MUST follow the procedure outlined in section [3.1.4](#Section_3.1.4) and MUST send a [Refresh](#Section_2.2.3.3) message to the [**resolver service**](#gt_resolver-service).

<a id="Section_3.1.4.6"></a>
#### 3.1.4.6 GetServiceInfo

The GetServiceInfo operation is used to query the referral policy of a Peer Channel protocol mesh. The [**resolver client**](#gt_resolver-client) MUST follow the procedure outlined in section [3.1.4](#Section_3.1.4) and MUST send a [GetServiceInfo](#Section_3.1.4.6) message to the [**resolver service**](#gt_resolver-service).

<a id="Section_3.1.5"></a>
### 3.1.5 Message Processing Events and Sequencing Rules

After an appropriate response message is received (for instance, [RegisterResponse](#Section_2.2.4.1) for a [Register](#Section_3.1.4.1) request, and so on), the [**resolver client**](#gt_resolver-client):
...
If the [**resolver client**](#gt_resolver-client) is not waiting for a response to a [Register](#Section_3.1.4.1) or [Update](#Section_2.2.3.4) message, it MUST ignore the [RegisterResponse](#Section_2.2.4.1) message.
...
If the [**resolver client**](#gt_resolver-client) is not waiting for a response to a [Resolve](#Section_2.2.3.2) message, it MUST ignore this message.
...
If the [**resolver client**](#gt_resolver-client) is not waiting for a response to a [Refresh](#Section_2.2.3.3) message, it MUST ignore this message.
...
If the custom [**resolver client**](#gt_resolver-client) is not waiting for a [ServiceSettingsResponseInfo](#Section_2.2.4.4) message, it MUST ignore this message.
...
There are no higher-layer triggered events for the [**resolver service**](#gt_resolver-service). All operations are confined to the processing of messages (section [3.2.5](#Section_3.2.5)) or the maintenance timer (section [3.2.2.1](#Section_3.2.2.1)).
...
- "S" sends a [RegisterResponse](#Section_2.2.4.1) message to "A" with the **RegistrationId** of the newly created [**registration record**](#gt_registration-record) (100).
...
- "A" sends a [Resolve](#Section_2.2.3.2) request to "S" (contents: "ExampleMesh", 5 for **MaxAddresses**).
